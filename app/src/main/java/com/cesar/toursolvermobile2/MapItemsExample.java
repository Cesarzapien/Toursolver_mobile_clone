...
public class MapItemsExample {
    ...
    public MapItemsExample(Context context, MapView mapView,List<PlannedOrder> plannedOrders,List<Order> orders) {
    ...
    public void showAnchoredMapMarkers() {
        ...
        // Asegúrate de que plannedOrders tenga al menos 2 elementos
        if (plannedOrders != null && plannedOrders.size() > 1) {
            // Empezamos desde el índice 1 (la segunda posición)
            for (int i = 1; i < plannedOrders.size(); i++) {
                PlannedOrder order = plannedOrders.get(i);
                double lat = order.getStopY();
                double lon = order.getStopX();
    ...
    private void addPOIMapMarker(GeoCoordinates geoCoordinates) {
        MapImage mapImage = MapImageFactory.fromResource(context.getResources(), R.drawable.marcador_accepted);
    ...
    private void addFlatMarker(GeoCoordinates geoCoordinates) {
        MapImage mapImage = MapImageFactory.fromResource(context.getResources(), R.drawable.baseline_location_accepted);
